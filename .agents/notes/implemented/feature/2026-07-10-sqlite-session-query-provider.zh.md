...
稳定观察要求在两次尝试内完成两次完全一致的全量语料快照比对（分别在检查每个变更会话之前与之后）。对于首次冷构建耗时数分钟的大规模且持续增长的语料库，这个窗口内可能发生快照抖动——例如某个长时间运行的会话恰好在扫描期间做检查点写入——导致两次尝试都以 `SESSION_QUERY_PERSISTENCE_FAILED: session-search persistence observation did not stabilize after one retry` 失败，后台预热与调用方超时短于扫描耗时的实时搜索调用均无法完成。这是用简单、无竞态的对齐模型换取大规模部署下的冷构建延迟；系统未为这种情况构建非全量重扫的对齐路径。
