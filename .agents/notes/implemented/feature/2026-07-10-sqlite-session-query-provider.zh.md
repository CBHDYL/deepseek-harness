...
稳定观察要求在两次尝试内满足：已列出的会话 id 成员保持不变（新增或删除始终触发重试，因此扫描期间出现或消失的会话不会被遗漏），并且这一轮实际检查过的每个会话——已建立索引且修订号匹配的会话会被缓存命中跳过、不参与比较——在重新列出的快照中仍具有相同的修订号与会话头。这一轮没有检查过的会话即使修订号发生变化，也不会强制重试：该会话自身的修订号会在它自己下一次被观察时被捕捉到。这使重试成本与本轮实际触达的工作量成正比，而不是与整个语料库规模成正比，因此大规模且持续写入的语料库中与本轮无关的抖动（例如某个长时间运行的会话恰好在扫描期间做检查点写入）本身不会阻止首次冷构建完成。
