...
稳定观察要求在两次尝试内，这一轮实际检查过的每个会话——已建立索引且修订号匹配的会话会被缓存命中跳过、不参与比较——在重新列出的快照中仍具有相同的修订号与会话头；这一轮没有检查过的会话即使修订号发生变化，也不会强制重试，因为该会话自身的修订号会在它自己下一次被观察时被捕捉到。这使重试成本与本轮实际触达的工作量成正比，而不是与整个语料库规模成正比，因此大规模且持续写入的语料库中与本轮无关的内容抖动本身不会阻止首次冷构建完成。在两次列表之间新出现的会话会被补入同一轮观察——通过最多四轮重新列表来检查并纳入——而不是让整轮作废，因为吸收一个新会话的成本与语料库规模无关；这使得冷构建耗时数分钟期间持续出现的少量新会话，不会让扫描反复从头重来，同时仍能保证一个会话在其创建后紧接着的那次搜索里就可见。某个此前已知、后来却从列表中消失的会话，始终会落入常规的整轮重试：与纯新增不同，本轮无法判断该 id 已检查过的内容是否仍是即将写入那一行的准确依据。若补入轮数预算耗尽后仍持续有会话出现，同样会落入常规的整轮重试，并在两次尝试都耗尽后彻底失败。
