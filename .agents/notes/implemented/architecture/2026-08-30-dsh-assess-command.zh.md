...
[English](2026-08-30-dsh-assess-command.md) | 中文

## Problem

`dsh` 只有配置/启动/插件三种模式，却缺少一个**不启动 tree、不写任何路径**的命令来回答「当前安装实际运行的是什么」：版本、node、profile 组合、patch layer digest、registry 通道与一组安全健康探针。此前这些事实分散在外部脚本与部署文档中，无法由一个命令绑定到同一运行身份；升级/回滚评估因此缺少可复现的 baseline。
