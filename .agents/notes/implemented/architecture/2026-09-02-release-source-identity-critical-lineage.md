...
- `scripts/release/gen-patch-manifest.ts` is the minimal patch manifest: explicit `--baseline`/`--candidate` revisions, computed merge base, every commit in the delta attributed through an explicit ownership table (PR1–PR6 / R1–R4 / ALIGN / SANITATION / STOPGAP / MIGRATION / OTHER), every changed file with its touching commits, and zero silently omitted files (a file no mapped commit touches is reported unmapped). Output is deterministic JSON + Markdown; it is evidence, never authority.
