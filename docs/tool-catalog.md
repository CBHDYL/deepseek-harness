...
| `@deepseek-ai/dsh-tool-browser` | `browser` | `ctx.tools`, `ctx.systemPrompt` | `tool/call`, `tool/result` | - | browser drives one headless Chromium page per session for smoke-level UI checks; the page and browser close when the session disposes. |
