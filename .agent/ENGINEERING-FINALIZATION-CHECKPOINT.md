...
POST-PROMOTION INCIDENT + RECURRENCE FIX (this session):
  INCIDENT: :3080 was DOWN in a launchd restart loop (runs=413, "spawn scheduled"). Not a browser/verify problem.
  ROOT CAUSE (forensic, by mtime): 17:23:03 all 265 entries of ~/.dsh/profiles/node_modules/@deepseek-ai were re-linked back into the GLOBAL NVM install in one bulk operation. dsh's own profile provisioning links @deepseek-ai/* to whichever runtime performs it, and every plain `dsh` / `dsh plugin` / `dsh-doctor provision` from a shell runs the GLOBAL binary — so any such command silently reverts the M5 anchor. verify-composition-anchor then fail-closed on every KeepAlive retry, i.e. the "sudden restarts then breakage" the user reported were the SYMPTOM of the refusal, not its cause. A separate track hot-patched the global install's known-event-types.js at 19:08 while the service had already been down since 17:23, so that patch never took effect. Two repair tracks were fighting.
  FIX 1 — launcher self-heal: dsh-m5-web-launch.sh now runs anchor-composition.sh --apply before the verifier, so a provisioning revert is auto-corrected instead of causing an outage. Proven by reproduction: --restore reverted 270 entries to global (verifier REFUSED), kickstart then self-healed and booted candidate cleanly (258/258 in-slot, 0 global-lineage).
  FIX 2 — launcher exit code: `if ! cmd; then rc=$?` captured the negation's status, reporting failures as "exit 0" and hiding them from launchd. Status is now captured directly.
  FIX 3 — anchor-composition.sh --restore was NON-FUNCTIONAL: it read into `path`, which in zsh is tied to the PATH array, so PATH was clobbered and the next `rm` failed with "command not found". set -e aborted at the first entry, so nothing was corrupted, but the documented emergency rollback would not have worked. Loop variable renamed to `link`; --restore now restores all 270 entries.
  STATE AT CLOSE: :3080 up on candidate stage-candidate-3 @ 114c8087ad…, GUI 401/403-token OK, 0 error lines. Affected session reads with 0 refused events; it now carries FOUR browser/verify events (785094, 785130, 840633, 840670) because it stayed in active use — the earlier byte-identical fixture baseline (52cf15fc…) is therefore superseded by normal appends, not damage.
  RESIDUAL: the global NVM install carries an unmanaged hot-patch (browser/verify added to its known-event-types.js at 19:08). It is not needed while M5 is live and is not tracked by any M5 manifest; recommend reverting it or recording it deliberately.
