 ...
 * parts of the envelope. An estimate can undercount against a provider
 * tokenizer — callers that need a hard safety boundary measure UTF-8 bytes
 * separately.
 * @param envelope - the assembled request fields.
 * @returns heuristic tokens for the complete request.
 */
export function estimateRequest(envelope) {
    let tokens = 0;
    for (const message of envelope.messages)
        tokens += estimateMessage(message);
    tokens += estimateSystemTokens(envelope.system === undefined ? undefined : { system: envelope.system });
