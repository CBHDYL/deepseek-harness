...
- **Safety classification is never taken from the server.** The bridge reads only name, description, input schema, supported output schema, and task requirement — `annotations.readOnlyHint` and any self-declared effects are dropped. A registered MCP tool therefore carries NO `effects` declaration: the action-policy guard always treats it as `undeclared` (side-effectful under enforce) and asks for approval, exactly like any other undeclared tool. The shipped effects catalog (`docs/tool-catalog.md`) harvests only shipped in-repo tool packages; MCP runtime registrations never enter it.
- **Model-facing metadata is byte-bounded at sync** (PR-6 source bounds): a tool whose description exceeds `maxToolDescriptionBytes` (default 4096) or whose serialized `inputSchema` + `outputSchema` exceed `maxToolSchemaBytes` (default 65536) is excluded from the generation with a warning — never silently truncated into the prompt. Both limits are deployment config with defaults.
