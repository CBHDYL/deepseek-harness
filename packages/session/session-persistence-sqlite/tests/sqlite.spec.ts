...
describe('SessionPersistenceSqlite physical packing', () => {
  ...
  it('rejects a stale repair without deleting a newer winning tail', async () => {
    ...
    await winner.commitRepair(header, [], 1, [], undefined)
    await winner.appendBatch(header, [chunk(1), chunk(2)], true)
    await expect(stale.commitRepair(header, [], 1, [], undefined)).rejects.toThrow(/repair is stale/)
...
describe('SessionPersistenceSqlite edge behavior', () => {
  ...
  it('keeps empty mutations inert and rolls back a repair without metadata', async () => {
    ...
    await store.commitRepair(header, [], undefined, [], undefined)
    expect(await store.readStoredRevision(header.id)).toBeUndefined()
    await expect(store.commitRepair(header, [], 0, [], undefined)).rejects.toThrow(/metadata row is missing/)
    await store.close()
  })
  ...
  it('rejects omitted torn markers and stale closer positions', async () => {
    ...
    db.prepare(testSql('insert-corrupt-event')).run(header.id, 1, 'assistant/chunk', 2, '{not json', null)
    db.close()
    await expect(store.commitRepair(header, [], undefined, [chunk(1)], undefined)).rejects.toThrow(/omitted current torn tail/)
    await store.commitRepair(header, [], 1, [], undefined)
    await expect(store.commitRepair(header, [], undefined, [chunk(2)], undefined)).rejects.toThrow(/repair batch starts at seq 2/)

    const cleared = new DatabaseSync(path)
    cleared.prepare(testSql('delete-session-events')).run(header.id)
    cleared.close()
    await store.commitRepair(header, [], undefined, [chunk(0)], undefined)
