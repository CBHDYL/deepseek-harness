...
import { StoredContentCorruptionError } from '@deepseek-ai/dsh-session-persistence'
...
export function scanZstdFrames(buffer: Buffer, maxFrames = Number.POSITIVE_INFINITY): ZstdFrameScan {
  ...
  while (offset < buffer.length) {
    ...
    if (buffer.readUInt32LE(offset) !== ZSTD_MAGIC) {
      throw new StoredContentCorruptionError(`corrupt Zstandard session log: invalid frame magic at byte ${offset}`)
    }
    offset += 4
    ...
    if ((descriptor & 0x18) !== 0) {
      throw new StoredContentCorruptionError(`corrupt Zstandard session log: reserved frame-header bit at byte ${offset - 1}`)
    }
    ...
    for (;;) {
      ...
      if (blockType === 0x03) {
        throw new StoredContentCorruptionError(`corrupt Zstandard session log: reserved block type at byte ${offset - 3}`)
      }
