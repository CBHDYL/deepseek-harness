...
同一会话的所有触发共享一条单飞队列：每会话最多一个检查点写在进行中，每个排队的写在轮到它时快照最新的注册表切面（后到的切面绝不会输给先到的），而在写入尚未取切面期间到达的触发合并进该任务。detach 是同一队列上的有序任务——它排在任何先前的写之后、从不被合并掉——且其最终切面在 detach 时刻同步捕获（卸载期间投影单元并发注销，任务执行时再取可能为空）。脏记账（`dirtyStats(session)`：待写事件数、连续失败数、剩余自动重试数）只由覆盖了这些事件的成功写入退休；失败的写让其事件保持脏、等待重试或下一触发，被拒绝的任务绝不搁浅队列。
