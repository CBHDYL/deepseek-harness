...
- **A backend `emit()` rejection withholds that one record, fail-closed** — the handoff cursor continues past it and the record is never re-handed (the E3 permanent test pins this). Telemetry is never canonical correctness state: a lost record costs observability, never session truth.
