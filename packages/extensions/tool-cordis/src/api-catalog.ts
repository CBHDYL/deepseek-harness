  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Durably checkpoint one live session NOW (all mandatory points call this; tests and carriers may too). The registry cut is snapshotted at this task\'s turn, then the session\'s record is replaced on the domain\'s write chain. NOT fail-soft — callers on the fail-soft paths contain it.',
        parameters: [{ name: 'session', description: 'the live session to checkpoint.' }, { name: '_trigger', description: 'the trigger name for diagnostics (unused: the enqueueing caller owns the failure log).' }, { name: 'final', description: 'the ordered detach task: skips the post-flush lifecycle recheck (the session is already detached; the last cut must land).' }, { name: 'captured', description: 'a cut snapshotted at detach time (the disposal cascade unregisters projection units while the task is queued, so a task-time checkpoint would be empty).' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'The local prompt budget refused the composed request BEFORE any provider dispatch.',
    description: 'The local prompt budget refused the composed request BEFORE any provider dispatch. The default action rejects (typed PROMPT_BUDGET_EXCEEDED); a listener that actually reduces the model-visible surface (compaction) returns `retry` so the loop rebuilds and re-measures. The loop bounds recovery retries per step (`budgetCompactionRetries`).',
