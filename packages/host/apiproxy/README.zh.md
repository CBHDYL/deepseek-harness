...
每条 mux 下行都持有一个有界的、每连接一份的发送队列（`DEFAULT_MAX_QUEUED_FRAMES`，10,000 帧），因此消费速度慢于宿主事件速率的 Consumer 不会让宿主内存无界增长。达到上限后，每次 push 丢弃最旧的一帧未投递帧，并铸造一个携带该连接累计 `dropped` 总数的 `session/resync` 帧。该标记帧保存在队列之外、不占用槽位，因此造成缺口的那个上限永远不会把报告缺口的帧本身丢掉；它先于那些在缺口中幸存下来的帧投递——客户端在应用任何位于缺失事件之后的内容之前，就已经知道自己的视图不完整。该帧以连接为作用域，不携带 `sessionId`：队列复用所有已挂载会话，且按年龄丢弃而不读取所丢内容，因此哪些会话丢了事件无法恢复，`dropped` 仅供诊断。客户端据此按历史重建每个已打开的会话，与重连所做的恢复相同，而连接本身保持存活。标记在投递后重新武装，因此之后的每次缺口都会再次上报。宿主（host）下行队列不铸造这类帧，仍然静默丢弃。
