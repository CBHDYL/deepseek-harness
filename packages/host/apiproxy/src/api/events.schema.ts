...
export const muxFrameSchema = z.discriminatedUnion('type', [
  ...
  // Positive by wire contract: the frame is minted only after a discard, so a
  // zero or negative total is host breakage and must fail here rather than
  // reach a client that would then rebuild for no reason.
  z.object({ type: z.literal('session/resync'), dropped: z.number().int().positive() }),
