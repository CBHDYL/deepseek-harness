...
数据库携带 application id、schema 版本 9，以及由会话格式版本和已知事件类型派生的读取方兼容性指纹。schema 版本或读取方指纹变化时，系统会重置已识别的派生 schema，避免仅因来源修订号相同而复用其他读取 lineage 生成的行。打开时会拒绝其他应用程序拥有的文件或规范数据库，并拒绝未知用户表，因此不相关或 session-persistence 数据库绝不会被触碰。在 POSIX 文件系统上，缺失的目录与数据库文件以仅所有者可访问的方式创建（进程 umask 前为 `0700` 与 `0600`）。每个派生索引路径在一个进程中只能由一个服务拥有；世代与 TEMP 遮蔽状态由连接持有。
