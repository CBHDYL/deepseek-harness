---
description: "The central action-policy interceptor gating side-effectful tool calls through the approval seam."
...
The `effects` classification is read from the SHIPPED `ToolDefinition` in the tool registry, never from MCP or other self-declared metadata: an MCP server claiming `readOnlyHint` (or any forged `effects` field) cannot downgrade the gate, and undeclared tools fold through `treatUndeclaredAsSideEffectful`. Model-authored dynamic Cordis tools go through the same rule — the host runner strips any `effects` the package code declares at the sandbox `defineTool` boundary, so a spoofed `read-only` can never exempt a side-effectful dynamic body. The declaration is self-attested by the same shipped code that owns `execute()`, matching the plugin trust boundary — runtime wrapper substitution against other tools is guarded at the dispatch boundary (see the PR-2 authoritative-execution-snapshot work in the remediation ledger). No invariant companion is published because the guard contributes no mutable relation of its own: every observable it enforces (classification, ask/deny folds, gating) is asserted by the ToolRuntime and approval suites at the dispatch seam.
