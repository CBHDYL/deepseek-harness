/**
 * Action-policy guard: a central mandatory action-policy interceptor (P-GUARD).
 * Today approval/sandbox governance only covers calls that OPT IN by
 * requesting approval inside the tool body — a tool or hook that performs a
 * sensitive action without requesting approval bypasses the policy entirely.
 * This guard moves the decision to `tools/pre-execute`: every tool call is
 * gated through the approval seam. Tools that declare `effects: 'read-only'`
 * on their shipped definition skip the gate; tools declaring
 * `'side-effectful'` and tools with no declaration (the
 * `treatUndeclaredAsSideEffectful` default) fold through it. MCP/self-declared
 * effects are never trusted: the classification is read from the tool
 * registry's shipped `ToolDefinition`.
 *
