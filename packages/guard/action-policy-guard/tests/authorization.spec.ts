...
async function harness(): Promise<Harness> {
  ...
  const agent = await ctx.agentLoop.create(SessionId('authz'), { provider: 'mock', model: 'mock' })
  return {
    ctx,
    agent,
    ran: () => ran,
...
describe('execution-attempt authorization', () => {
  ...
  it('the audit chain joins asked → decided → tool/call → tool/result on one operationId (A6)', async () => {
    ...
    h.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'go' }], source: { kind: 'user' } }))
    await waitForIdle(h.ctx, h.agent)
    const asked = h.asked()[0]?.data as { operationId?: string; id?: unknown }
    const decided = h.decided()[0]?.data as { operationId?: string; id?: unknown }
    const call = h.agent.session.snapshotEvents().find(event => event.type === 'tool/call')
    const result = h.agent.session.snapshotEvents().find(event => event.type === 'tool/result')
    expect(asked?.operationId).toBeTruthy()
    // The decided row joins by approval id (operationId on decided is a
    // deferred P2); the call/result rows join on the operation identity.
    expect(decided?.id).toBe(asked?.id)
    expect(call?.type === 'tool/call' && (call.data as { operationId?: string }).operationId).toBe(asked?.operationId)
    expect(result?.type === 'tool/result' && (result.data as { operationId?: string }).operationId).toBe(asked?.operationId)
  ...
  it('cross-invariant B: an authorized execution persists and survives reload with its full durable chain', async () => {
    ...
    const agent = await ctx.agentLoop.create(SessionId('cross-b'), { provider: 'mock', model: 'mock' })
    ...
    const operationId = call?.type === 'tool/call' ? (call.data as { operationId?: string }).operationId : undefined
    expect(operationId).toBeTruthy()
    expect(result?.type === 'tool/result' && (result.data as { operationId?: string }).operationId).toBe(operationId)
    ...
    const loadedHandle = await ctx2.sessionPersistence.open(SessionId('cross-b'), 'read')
    const loadedEvents = await loadedHandle.read()
    await loadedHandle.close()
    const loadedCall = loadedEvents.find(event => event.type === 'tool/call')
    const loadedResult = loadedEvents.find(event => event.type === 'tool/result')
    expect(loadedCall?.type === 'tool/call' && (loadedCall.data as { operationId?: string }).operationId).toBe(operationId)
    expect(loadedResult?.type === 'tool/result' && (loadedResult.data as { operationId?: string }).operationId).toBe(operationId)
